...
describe("test scaffold", () => {
	...
	it("seed data: localities", async () => {
		const r = await env.leadgen
			.prepare("SELECT COUNT(*) as cnt FROM localities")
			.first<{ cnt: number }>();
		expect(r?.cnt).toBe(4);
	});
	...
	it("seed data: sellers", async () => {
		const r = await env.leadgen
			.prepare("SELECT COUNT(*) as cnt FROM sellers")
			.first<{ cnt: number }>();
		expect(r?.cnt).toBe(2);
	});
	...
	it("seed data: call_log", async () => {
		const r = await env.leadgen
			.prepare("SELECT COUNT(*) as cnt FROM call_log")
			.first<{ cnt: number }>();
		expect(r?.cnt).toBe(4);
	});
	...
	it("seed data: business_owners", async () => {
		const r = await env.leadgen
			.prepare("SELECT COUNT(*) as cnt FROM business_owners")
			.first<{ cnt: number }>();
		expect(r?.cnt).toBe(2);
	});
    ...
    `)
			.bind(TEST_IDS.sellers.jan, TEST_IDS.businesses.hydraulikWarszawa)
			.first<{ status: string }>();

...
export function createSellerStartHandler(
  ...
  return {
    ...
    handle: async (ctx) => {
      ...
      const seller = await ctx.env.leadgen.prepare(
        `SELECT id, ${column} FROM sellers WHERE token = ?`,
      ).bind(sellerToken).first<{ id: number; [k: string]: unknown }>();
      ...
      await ctx.env.leadgen.prepare(
        `UPDATE sellers SET ${column} = ? WHERE token = ?`,
      ).bind(ctx.chatId, sellerToken).run();

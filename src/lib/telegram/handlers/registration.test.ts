...
function msgUpdate(text: string): TelegramUpdate {
  return {
    update_id: 1,
    message: { message_id: 1, chat: { id: 999, type: 'private' }, text, date: 0 },
  };
}

describe('createSellerStartHandler (report_chat_id)', () => {
  const handler = createSellerStartHandler('report_chat_id', 'Bedziesz otrzymywac codzienne raporty.');
  ...
  it('replies with help when no token provided', async () => {
    ...
    expect(ctx.reply).toHaveBeenCalledWith('Uzyj linku rejestracyjnego od administratora.');
  });
  ...
  it('registers seller with valid token', async () => {
    ...
    expect(ctx.reply).toHaveBeenCalledWith(expect.stringContaining('Zarejestrowano'));

    const seller = await env.leadgen.prepare(
      'SELECT report_chat_id FROM sellers WHERE token = ?'
    ).bind(TEST_IDS.tokens.sellerAnna).first<{ report_chat_id: string }>();
...
describe('createSellerStartHandler (notify_chat_id)', () => {
  const handler = createSellerStartHandler('notify_chat_id', 'Bedziesz otrzymywac powiadomienia o nowych kontaktach.');
  ...
  it('registers seller notify_chat_id', async () => {
    ...
    const seller = await env.leadgen.prepare(
      'SELECT notify_chat_id FROM sellers WHERE token = ?'
    ).bind(TEST_IDS.tokens.sellerJan).first<{ notify_chat_id: string }>();

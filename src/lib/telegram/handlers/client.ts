import type { TgHandler, TgContext } from '../types';
import type { SiteData } from '../../../types/site';
import { escapeHtml } from '../../telegram';
import { getSite, putSite, promoteDraft, deleteSite } from '../../site-store';
...
export const startLinkHandler: TgHandler = {
  ...
  handle: async (ctx) => {
    ...
    const owner = await ctx.env.leadgen.prepare(
      'SELECT id, business_id, chat_id FROM business_owners WHERE token = ?',
    ).bind(bizToken).first<{ id: number; business_id: number; chat_id: string | null }>();
    ...
    if (owner.chat_id && owner.chat_id === ctx.chatId) {
      await ctx.reply('Juz jestes polaczony. Wyslij wiadomosc aby edytowac wizytowke.');
      return;
    }

    await ctx.env.leadgen.prepare(
      'UPDATE business_owners SET chat_id = ? WHERE token = ?',
    ).bind(ctx.chatId, bizToken).run();

    const biz = await ctx.env.leadgen.prepare(
...
export const draftCallbackHandler: TgHandler = {
  ...
  handle: async (ctx) => {
    ...
    ).bind(ctx.chatId).first<{ business_id: number }>();
    ...
    const loc = await ctx.env.leadgen.prepare(
...
async function defaultEditDeps(apiKey: string): Promise<EditDeps> {
  const { createGLM5, editContent, summarizeChanges } = await import('../../site-content');
  const llm = createGLM5(apiKey);
  return {
    editContent: (current, instruction) => editContent(llm, current, instruction) as Promise<SiteData>,
    summarizeChanges: summarizeChanges as (old: SiteData, updated: SiteData) => string,
  };
}
...
function createOwnerEditHandle(deps?: EditDeps) {
  return async (ctx: TgContext): Promise<void> => {
    ...
    const currentSite = await getSite(ctx.env.sites, 'live', loc!.slug, biz!.slug);
    if (!currentSite) {
    ...
    try {
      ...
      await putSite(ctx.env.sites, 'draft', loc!.slug, biz!.slug, patchedSiteData as SiteData);
...
export const ownerEditHandler: TgHandler & { withDeps?: (deps: EditDeps) => TgHandler } = {
  match: (update) => !!update.message?.text && !update.message.text.startsWith('/'),

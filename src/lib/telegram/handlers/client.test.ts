...
describe("startLinkHandler", () => {
	...
	it("matches /start biz_* messages", () => {
		expect(startLinkHandler.match(msgUpdate("/start biz_abc"), makeCtx())).toBe(
			true,
		);
	});
	...
	it("does not match /start without biz_ prefix", () => {
		expect(
			startLinkHandler.match(msgUpdate("/start seller_token"), makeCtx()),
		).toBe(false);
	});
	...
	it("links owner chat_id with valid token", async () => {
		...
		expect(ctx.reply).toHaveBeenCalledWith(
			expect.stringContaining("Polaczono"),
		);

		const owner = await env.leadgen
			.prepare("SELECT chat_id FROM business_owners WHERE token = ?")
			.bind(TEST_IDS.tokens.bizPiekarnia)
	...
	it("replies idempotent when already linked", async () => {
		...
		await startLinkHandler.handle(ctx);
		expect(ctx.reply).toHaveBeenCalledWith(
			expect.stringContaining("Juz jestes polaczony"),
		);
	});
});
...
describe("draftCallbackHandler", () => {
	...
	it("matches approve: callback data", () => {
		expect(draftCallbackHandler.match(cbUpdate("approve:1"), makeCtx())).toBe(
			true,
		);
	});

	it("matches reject: callback data", () => {
		expect(draftCallbackHandler.match(cbUpdate("reject:1"), makeCtx())).toBe(
			true,
		);
	});

	it("does not match other callback data", () => {
		expect(draftCallbackHandler.match(cbUpdate("other:1"), makeCtx())).toBe(
			false,
		);
	});
	...
	it("denies access when no owner found", async () => {
		...
		expect(ctx.reply).toHaveBeenCalledWith(
			expect.stringContaining("Brak dostepu"),
		);
	});
	...
	it("reject deletes draft", async () => {
		...
		expect(ctx.reply).toHaveBeenCalledWith(
			expect.stringContaining("odrzucone"),
		);
...
describe("ownerEditHandler", () => {
	...
	it("matches regular text messages", () => {
		expect(ownerEditHandler.match(msgUpdate("zmien adres"), makeCtx())).toBe(
			true,
		);
	});
	...
	it("replies error when site not generated yet", async () => {
		...
		await ownerEditHandler.handle(ctx);
		expect(ctx.reply).toHaveBeenCalledWith(
			"Wizytowka jeszcze nie zostala wygenerowana.",
		);
	});
	...
	it("replies generic error for non-LLM failures", async () => {
		...
		expect(ctx.reply).toHaveBeenCalledWith(
			"Nie udalo sie przetworzyc zmian. Sprobuj ponownie.",
		);
	});
});

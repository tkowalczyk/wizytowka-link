...
import type { TgHandler, BotConfig } from './types';
...
function makeUpdate(overrides: Partial<TelegramUpdate> = {}): TelegramUpdate {
  return {
    update_id: 1,
    message: { message_id: 1, chat: { id: 42, type: 'private' }, text: 'hello', date: 0 },
    ...overrides,
  };
}
...
describe('createBotRoute', () => {
  ...
  it('dispatches to first matching handler', async () => {
    ...
    const h1: TgHandler = {
      match: () => true,
      handle: async () => { handled.push('h1'); },
    };
    const h2: TgHandler = {
      match: () => true,
      handle: async () => { handled.push('h2'); },
    };
  ...
  it('catches handler errors and replies with fallback', async () => {
    ...
    try {
      ...
      const sendCall = calls.find((c: any[]) => c[0]?.includes?.('/sendMessage'));

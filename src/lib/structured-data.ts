...
export function buildLocalBusinessLd(biz: BizData, url: string) {
  return {
    ...
    ...(biz.rating && biz.reviews_count
      ? { aggregateRating: { '@type': 'AggregateRating', ratingValue: biz.rating, bestRating: 5, ratingCount: biz.reviews_count } }
      : {}),
    url,
  };
}

export function buildBreadcrumbLd(seoTitle: string, locSlug: string, locName: string) {
  return {
    '@context': 'https://schema.org',
    '@type': 'BreadcrumbList',
    itemListElement: [

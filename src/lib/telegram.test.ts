...
function baseStats(
	overrides: Partial<DailyReportStats> = {},
): DailyReportStats {
	return {
...
describe("formatDailyReport", () => {
	...
	it("omits banner when quota flag is false", () => {
		const result = formatDailyReport(
			SELLER,
			baseStats({ quotaExhausted: false }),
			DATE,
		);
...
describe("sendCriticalAlert", () => {
	...
	it("sends to all sellers with notify_chat_id (skipping NULL)", async () => {
		...
		try {
			const result = await sendCriticalAlert(env, {
				kind: "quota",
	...
	it("debounces second call within 6h for same kind", async () => {
		...
		await env.leadgen
			.prepare(
				"INSERT INTO alert_log (kind, sent_at) VALUES ('quota', datetime('now', '-1 hour'))",
			)
			.run();
	...
	it("sends again after debounce window has elapsed", async () => {
		...
		await env.leadgen
			.prepare(
				"INSERT INTO alert_log (kind, sent_at) VALUES ('quota', datetime('now', '-7 hours'))",
			)
			.run();
	...
	it("debounce is per-kind: different kind sends even if another kind is fresh", async () => {
		...
		await env.leadgen
			.prepare(
				"INSERT INTO alert_log (kind, sent_at) VALUES ('quota', datetime('now'))",
			)
			.run();
...
describe("dispatchCriticalAlert", () => {
	...
	it("quotaExhausted=true triggers a quota alert via waitUntil", async () => {
		...
		try {
			...
			const sent = calls.find(
				(c) => typeof c[0] === "string" && c[0].includes("/sendMessage"),
			);
	...
	it.each([
		"auth",
		"payment",
	] as const)("errorKind=%s triggers an alert with that kind", async (kind) => {
		const fetchMock = mockTelegramFetch();
		const origFetch = globalThis.fetch;
		globalThis.fetch = fetchMock as unknown as typeof fetch;

...
import { geocodeLocalities, type GeocoderDeps, type GeoResult } from './geocoder';
...
describe('geocodeLocalities', () => {
  ...
  it('skips localities with future geocode_retry_after', async () => {
    await env.leadgen
      .prepare("UPDATE localities SET geocode_retry_after = datetime('now', '+1 day') WHERE id = 4")
      .run();
  ...
  it('picks up localities with past geocode_retry_after', async () => {
    await env.leadgen
      .prepare("UPDATE localities SET geocode_retry_after = datetime('now', '-1 hour') WHERE id = 4")
      .run();
  ...
  it('increases backoff on repeated failures', async () => {
    ...
    const row = await env.leadgen
      .prepare('SELECT geocode_retry_after, geocode_fail_count FROM localities WHERE id = 4')
  ...
  it('respects geocode_failed=1 as manual skip', async () => {
    await env.leadgen
      .prepare('UPDATE localities SET lat = NULL, geocode_failed = 1 WHERE id = 4')
      .run();

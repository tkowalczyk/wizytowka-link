...
import { preflightDiscovery, SKIP_FLAG_KEY, SKIP_TTL_SECONDS } from './preflight';
...
describe('preflightDiscovery', () => {
  it('account fetch fails → does NOT set skip (fail open)', async () => {
    ...
    const result = await preflightDiscovery({
      fetchAccount: async () => fakeAccount({
        status: 'error',
        searches_left: null,
  ...
  it('searches_left below threshold → sets skip flag in KV with TTL', async () => {
    ...
    const payload = JSON.parse(store.get(SKIP_FLAG_KEY)!) as { searchesLeft: number };

...
function fakeResult(overrides: Partial<SerpApiLocalResult> = {}): SerpApiLocalResult {
  return {
    position: 1,
    title: 'Test Biznes',
...
describe('discovery: runDiscovery', () => {
  ...
  it('tracer bullet: 1 search result → 1 business inserted', async () => {
    ...
    const row = await env.leadgen
      .prepare('SELECT title, place_id, phone, locality_id FROM businesses WHERE place_id = ?')
      .bind('place_test_001')
      .first<{ title: string; place_id: string; phone: string; locality_id: number }>();
  ...
  it('multi-locality retry: 0 leads in first locality → tries next', async () => {
    ...
    await env.leadgen.prepare("UPDATE businesses SET created_at = datetime('now', '-1 day')").run();
  ...
  it('report: NotifyPort.reportToSellers called with correct stats', async () => {
    ...
    const notify: NotifyPort = {
      reportToSellers: async (stats) => { reportedStats = stats; },
    };

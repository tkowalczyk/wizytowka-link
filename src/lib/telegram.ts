...
export function formatDailyReport(
	...
	const remaining =
		stats.new_leads - Math.min(stats.top_leads.length, MAX_TOP_LEADS);
...
export async function sendCriticalAlert(
	...
	const subscribers = await env.leadgen
		.prepare(
			"SELECT notify_chat_id FROM sellers WHERE notify_chat_id IS NOT NULL",
		)
	...
	for (const sub of subscribers.results) {
		try {
			...
			console.log(
				`telegram: critical alert failed for chat ${sub.notify_chat_id}: ${err}`,
			);
		}
	}
...
export function dispatchCriticalAlert(
	...
	const errorKind = meta.errorKind as
		| SerpApiErrorKind
		| "preflight-skip"
		| null
		| undefined;
	...
	if (!panelUrl) {
		console.error(
			"[critical-alert] ADMIN_PANEL_URL not configured, skipping alert dispatch",
		);
		return;
	}
	...
	const message = formatCriticalAlertMessage(kind, panelUrl, {
		searchesLeft:
			typeof meta.searchesLeft === "number" ? meta.searchesLeft : null,

...
import { buildLocalBusinessLd, buildBreadcrumbLd, type BizData } from './structured-data';
...
describe('buildLocalBusinessLd', () => {
  it('includes ratingCount when both rating and reviews_count present', () => {
    const ld = buildLocalBusinessLd(baseBiz, 'https://wizytowka.link/zabki/bistro');
  ...
  it('omits aggregateRating when reviews_count is null', () => {
    const ld = buildLocalBusinessLd({ ...baseBiz, reviews_count: null }, 'https://wizytowka.link/zabki/bistro');
    expect(ld).not.toHaveProperty('aggregateRating');
  });

  it('omits aggregateRating when rating is null', () => {
    const ld = buildLocalBusinessLd({ ...baseBiz, rating: null }, 'https://wizytowka.link/zabki/bistro');
    expect(ld).not.toHaveProperty('aggregateRating');
  });

  it('omits aggregateRating when both null', () => {
    const ld = buildLocalBusinessLd({ ...baseBiz, rating: null, reviews_count: null }, 'https://wizytowka.link/zabki/bistro');
    expect(ld).not.toHaveProperty('aggregateRating');
  });
...
describe('buildBreadcrumbLd', () => {
  it('builds 3-level breadcrumb', () => {
    ...
    expect(ld.itemListElement[1].item).toBe('https://wizytowka.link/zabki-0921958');

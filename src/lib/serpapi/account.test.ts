...
describe('fetchSerpApiAccount', () => {
  ...
  it('never leaks api_key into the result, even on error', async () => {
    ...
    // Happy path
    const okFetch = async () =>
      okResponse({ plan_id: 'dev', total_searches_left: 1000, plan_searches_left: 1000 });
    ...
    const throwFetch = async () => {
      throw new Error(`connect ECONNREFUSED https://serpapi.com/account.json?api_key=${SECRET}`);
    };

...
import { validateContent, generateContent, editContent, summarizeChanges } from './site-content';
...
describe('validateContent', () => {
  ...
  it('throws on empty services', () => {
    const data = JSON.parse(VALID_JSON);
    data.services = [];
    expect(() => validateContent(JSON.stringify(data))).toThrow('invalid services');
  });
...
describe('editContent', () => {
  it('preserves original phone even if LLM changes it', async () => {
    const edited = { ...CURRENT, hero: { headline: 'Nowy', subheadline: 'Sub' } };
    (edited as Record<string, unknown>).contact = { cta_text: 'Zadzwoń', phone: 'HACKED', address: 'ul. X' };
...
describe('summarizeChanges', () => {
  it('reports headline change', () => {
    const updated: SiteContent = { ...CURRENT, hero: { headline: 'Nowy', subheadline: 'Sub' } };

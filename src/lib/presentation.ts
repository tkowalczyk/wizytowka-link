...
import type { LayoutVariant, StyleVariant, ThemeConfig } from "./themes";
import { getPaletteById, resolveTheme } from "./themes";

export function resolveFullTheme(
	slug: string,
	category: string,
	overrides: PresentationOverrides | null,
): ThemeConfig {
...
export async function getOverrides(
	...
	if (
		!row ||
		(!row.palette_override && !row.layout_override && !row.style_override)
	)
		return null;

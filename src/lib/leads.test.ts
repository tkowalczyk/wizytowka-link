...
describe("businessExists", () => {
	it("returns true for existing business", async () => {
		expect(
			await leads.businessExists(TEST_IDS.businesses.hydraulikWarszawa),
		).toBe(true);
	});
...
describe("list", () => {
	...
	it("sorts by status when requested", async () => {
		...
		const statusPage = await leads.list(TEST_IDS.sellers.jan, {
			sort: "status",
		});
	...
	it("paginates correctly", async () => {
		const page1 = await leads.list(TEST_IDS.sellers.jan, {
			pageSize: 2,
			page: 1,
		});
		...
		const page2 = await leads.list(TEST_IDS.sellers.jan, {
			pageSize: 2,
			page: 2,
		});
		...
		const page3 = await leads.list(TEST_IDS.sellers.jan, {
			pageSize: 2,
			page: 3,
		});
...
describe("logStatus", () => {
	...
	it("does not overwrite previous entries", async () => {
		...
		const rows = await env.leadgen
			.prepare(
				"SELECT * FROM call_log WHERE business_id = ? AND seller_id = ? ORDER BY created_at",
			)
...
describe("updateComment", () => {
	it("updates only the latest call_log entry", async () => {
		...
		const before = await env.leadgen
			.prepare(
				"SELECT id, comment FROM call_log WHERE business_id = ? AND seller_id = ? ORDER BY created_at DESC",
			)
...
describe("ensureOwnerToken", () => {
	it("returns existing token for business with one", async () => {
		const token = await leads.ensureOwnerToken(
			TEST_IDS.businesses.hydraulikWarszawa,
		);

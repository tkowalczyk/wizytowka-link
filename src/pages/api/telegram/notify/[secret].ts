...
export const POST: APIRoute = async ({ params, request, locals }) => {
  ...
  if (params.secret !== env.TG_NOTIFY_WEBHOOK_SECRET) {
    return new Response('forbidden', { status: 403 });
  }

  const token = env.TG_NOTIFY_BOT_TOKEN;
  ...
  const seller = await env.leadgen.prepare(
    'SELECT id, notify_chat_id FROM sellers WHERE token = ?'
  ).bind(sellerToken).first<{ id: number; notify_chat_id: string | null }>();
  ...
  if (seller.notify_chat_id === chatId) {
    await sendReply(token, chatId, 'Juz jestes zarejestrowany.');
    return new Response('ok');
  }

  await env.leadgen.prepare(
    'UPDATE sellers SET notify_chat_id = ? WHERE token = ?'
  ).bind(chatId, sellerToken).run();

  await sendReply(token, chatId, 'Zarejestrowano! Bedziesz otrzymywac powiadomienia o nowych kontaktach.');
  return new Response('ok');
};

...
function enrichRows(
	...
	return rows.map((r) => {
		...
		if (!wojName)
			console.warn(`TERC miss: woj=${r.woj} for ${r.nazwa} (sym=${r.sym})`);
...
async function main() {
	...
	console.log(
		`Stan: ${state.completedWoj.length}/16 woj, ${state.totalInserted} wstawionych`,
	);
	...
	for (const woj of WOJ_CODES) {
		...
		for (let i = 0; i < batches.length; i++) {
			...
			execSync(
				`pnpm wrangler d1 execute leadgen --remote --yes --file=${file}`,
				{
					stdio: "inherit",
				},
			);

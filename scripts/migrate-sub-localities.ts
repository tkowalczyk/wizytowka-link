  ...
  `);
	...
	for (const biz of subs) {
		...
		if (lat == null || lng == null || Number.isNaN(lat) || Number.isNaN(lng)) {
			console.warn(
				`  SKIP biz ${biz.biz_id} (${biz.biz_slug}): no GPS coords available`,
			);
			failed++;
			continue;
		}
    ...
    `);
      ...
      `);
		...
		if (!candidates.length) {
			console.warn(
				`  SKIP biz ${biz.biz_id} (${biz.biz_slug}): no MAIN locality found nearby`,
			);
			failed++;
			continue;
		}
		...
		if (biz.site_generated) {
			...
			try {
				...
				execSync(`pnpm wrangler r2 object delete leadgen-sites/${oldKey}`, {
					stdio: ["pipe", "pipe", "pipe"],
				});
			} catch (_e) {
		...
		d1(
			`UPDATE businesses SET locality_id = ${best.id}${slugUpdate} WHERE id = ${biz.biz_id}`,
		);

		console.log(
			`  ${biz.biz_id} ${biz.biz_slug}: ${biz.loc_slug} -> ${best.slug} (${bestDist.toFixed(1)}km)`,
		);
		migrated++;
	}
	...
	const remaining = d1Json<{ cnt: number }>(
		"SELECT COUNT(*) as cnt FROM localities WHERE sym <> sym_pod",
	);
